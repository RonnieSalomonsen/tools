...
try:
    import pefile
    if "get_imphash" not in dir(pefile.PE):
        print "pefile with imphash not installed. see https://github.com/erocarrera/pefile"
        sys.exit()
    import peutils
except ImportError:
    print 'pefile not installed, see https://github.com/erocarrera/pefile'
    sys.exit()
...
try:
    ...
    print 'PyRichHeader is not installed, see https://github.com/CIRCL/PyRichHeader'
...
class PEScanner:
    def __init__(self, files, yara_rules=None, peid_sigs=None, compidFile=None):
        self.files = files
    ...
    def collect(self):
        ...
        for file in self.files:
            ...
            out.append("imphash: %s" % pe.get_imphash())
            ...
            if self.compid:
                # Load comp_id.txt into dictionary
                descriptions = {}
                with open(self.compid) as f:
                    for line in f:
                        if len(line) > 8 and line[0] != '#':
                            compid = line[0:8]
                            desc = line[9:]
                            descriptions[compid] = desc
                        if 'str' in line:
                            break
                # Parse Rich Header
                try:
                ...
                s = "RichHdr: "
                if not valid:
                    s += "Checksum not valid! [SUSPICIOUS]"
                else:
                    s += "Checksum is valid!"
                out.append(s)
             
                out.append(self.header("Rich Header"))
                out.append("%-14s %-12s %-12s %-12s %-12s" % ("@Comp.id", "ID", "Version", "Count", "Description"))
                out.append("-" * 80)

                for compid, count in compids:
                    ids = "%08x" % compid
                    version = compid & 0x0000FFFF
                    description = ""
                    try:
                        description = descriptions[ids]
                    except:
                        description = "[---] Unknown"   
                    s = "0x%-12.08x %-12s %-12d %-12d %-12s" % (compid, ids[2:4], version, count, description)
                    out.append(s.rstrip())
...
if __name__ == "__main__":
    ...
    # You should fill these in with a path to your YARA rules and PEiD database and comp_id.txt file for richheader
    pescan = PEScanner(files, '', '', '')
    #pescan = PEScanner(files, '/<path>/peid.yara', '/<path>/UserDB.txt', '/<path>/comp_id.txt')
    pescan.collect()
